...
- [x] 1.3 Vendored a static `arm64` `ffmpeg` (osxexperts 8.1) via `vendor-ffmpeg.sh`. Has `libx264` ✓ and is self-contained (links only system frameworks). **Caveat:** this build lacks `libzimg`/`zscale`, so HDR tonemapping isn't available with it (SDR is fully covered) — a libzimg build is a follow-up for true HDR.
- [x] 1.4 Bundled `ffmpeg` at `Contents/Resources/ffmpeg`, executable bit set, code-signed. *(Verified: app reports it uses the bundled binary.)*
...
- [x] 3.3 HDR branch: `zscale`+`tonemap` chain implemented, **with graceful degradation** — `FFmpegCapabilities` detects whether the resolved ffmpeg has `zscale` and falls back to a plain SDR conversion (still in-spec/postable) when it doesn't. *(True tonemap unverified — current bundled build lacks `zscale`; needs a libzimg ffmpeg + HDR sample.)*
...
- [x] 5.3 Self-contained + offline ✓ verified — the packaged app converts using its own bundled ffmpeg (`…/Contents/Resources/ffmpeg (bundled)`), no network, no system ffmpeg needed.

## 6. Verification

- [x] 6.1 Converted the real `3024×1964 @ 120fps` screen recording → `1280×832`, 30fps, H.264 High, `yuv420p`, AAC. ✓
- [x] 6.2 No-audio → silent AAC track ✓ verified. HDR path degrades gracefully to SDR when `zscale` absent (see 3.3); true HDR tonemap pending a libzimg build.
